		...
		<section class="contents mb-4">
			...
			<div class="musicInfo mt-3">
				...
				<div class="artist d-flex border border-success p-3 mt-2">
					<div class="image">
						<img width="150" src="<%= musicInfo.get("thumbnail") %>">
					</div>
					<div class="info ml-3">
						<h3><%= musicInfo.get("title") %></h3>
						<div class="text-success font-weight-bold"><%= musicInfo.get("singer") %></div>	
							<div>앨범 <%= musicInfo.get("album") %></div>
							<div>재생시간 <%=min %>:<%=second %></div> 
							<div>작곡가 <%=musicInfo.get("composer") %></div>	
							<div>작사가 <%=musicInfo.get("lyricist") %></div>	
					</div>
				</div>
			</div>			
			<h3 class="mt-3">가사</h3> 
